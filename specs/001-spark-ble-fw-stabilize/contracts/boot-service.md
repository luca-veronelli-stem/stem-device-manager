...
- **Q1 (retry bound)**: each step retries at most `RetryBudget` times (default 3, configurable via the `BootService` constructor); the step then transitions to `Failed`. Exception (2026-06-10 bench finding): the **first** `CMD_PROGRAM_BLOCK` of each upload is bounded by `FirstBlockRetryBudget` instead (default 60, constructor-configurable) — the SPARK HMI bootloader acks `CMD_START_PROCEDURE` immediately but stays silent on block 0 until the target-area flash erase completes (~13 s measured for the 651 KB application area; ~110 s estimated for the 5.5 MB images area). The bound stays finite and constructor-set; blocks 1+ keep `RetryBudget`.

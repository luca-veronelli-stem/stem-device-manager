...
- **Decision**: Adopt `leanprover/lean4:v4.15.0` as the `lean-toolchain` pin. Use Lake as the build tool with a root `lakefile.lean` at `Lean/lakefile.lean`. Add mathlib4 as a dependency for state-machine / relational-algebra lemmas. Lean sources organized by spec id under `Lean/SpecNNN/`; this feature creates `Lean/Spec001/` (the earlier refactor-phase taxonomy — `Lean/Phase1/` described aspirationally in CLAUDE.md — was never committed and is retired).
